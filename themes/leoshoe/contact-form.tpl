...
{if isset($confirmation)}
	...
	<ul class="footer_links clearfix">
		<li><a href="{$base_dir}"><span class="icon-home"></span>{l s='Home'}</a></li>
	</ul>
...
{else}
	...
	<form action="{$request_uri|escape:'htmlall':'UTF-8'}" method="post" class="std form-horizontal" enctype="multipart/form-data">
		<div class="panel panel-default">
			<h3 class="panel-heading">{l s='send a message'}</h3>
			<div class="panel-body">
				<div class="select form-group">
					<label class="control-label col-sm-4" for="id_contact">{l s='Subject Heading'}</label>
					<div class="col-sm-6">
						{if isset($customerThread.id_contact)}
							{foreach from=$contacts item=contact}
								{if $contact.id_contact == $customerThread.id_contact}
									<input class="form-control" type="text" id="contact_name" name="contact_name" value="{$contact.name|escape:'htmlall':'UTF-8'}" readonly="readonly" />
				...
				<div class="form-group text">
					<label class="control-label col-sm-4" for="email">{l s='Email address'}</label>
					<div class="col-sm-6">
					{if isset($customerThread.email)}
						<input class="form-control" type="text" id="email" name="from" value="{$customerThread.email|escape:'htmlall':'UTF-8'}" readonly="readonly" />
					{else}
						<input class="form-control" type="text" id="email" name="from" value="{$email|escape:'htmlall':'UTF-8'}" />
					{/if}
					</div>
				</div>
			{if !$PS_CATALOG_MODE}
				...
				<div class="form-group text select">
					<label class="control-label col-sm-4" for="id_order">{l s='Order reference'}</label>
					<div class="col-sm-6">
				...
				<div class="form-group text select">
					{if !isset($customerThread.id_product)}
						<label class="control-label col-sm-4" for="id_product">{l s='Product'}</label>
						<div class="col-sm-6">
			...
			{if $fileupload == 1}
				<div class="form-group text">
					<label class="control-label col-sm-4" for="fileUpload">{l s='Attach File'}</label>
					<div class="col-sm-6">
			...
			<div class="form-group textarea">
				<label class="control-label col-sm-4" for="message">{l s='Message'}</label>
				<div class="col-sm-6">
					<textarea class="form-control" id="message" name="message" rows="5">{if isset($message)}{$message|escape:'htmlall':'UTF-8'|stripslashes}{/if}</textarea>
				</div>
			</div>
