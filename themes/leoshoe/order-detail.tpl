...
<div class="table_block">
	<table class="detail_step_by_step std table-responsive">
		<thead>
			<tr>
...
<div class="adresses_bloc row block clearfix">
	<ul class="address item {if $order->isVirtual()}col-sm-12{else}col-sm-6{/if}">
	...
	<ul class="col-sm-6 address alternate_item" {if $order->isVirtual()}style="display:none;"{/if}>
...
<div id="order-detail-content" class="table_block">
	<table class="std table-bordered table-responsive">
		<thead>
			<tr>
		...
		<tfoot>
			...
			<tr class="item">
				<td colspan="{if $return_allowed || $order->hasProductReturned()}{if $order->hasProductReturned() && $return_allowed}7{else}6{/if}{else}5{/if}">
					{l s='Total vouchers:'} <span class="price-discount price">{displayWtPriceWithCurrency price=$order->total_discounts currency=$currency convert=1}</span>
				</td>
			</tr>
			{/if}
...
{if !$is_guest}
	...
	<form action="{$link->getPageLink('order-detail', true)|escape:'html'}" method="post" class="std" id="sendOrderMessage">
		...
		<div class="form-group">
			<label class="control-lable" for="id_product">{l s='Product'}</label>
		</div>
		<div class="form-group">
		...
		<p class="textarea">
			<textarea class="form-control" rows="4" name="msgText"></textarea>
		</p>
		<p class="submit">
