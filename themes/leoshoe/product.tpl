...
<div id="primary_block" class="row block clearfix">
	...
	<div id="pb-right-column" class="col-md-5 col-sm-6">
		...
		{if $have_image}
			<span id="view_full_size">
				<img src="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'large_default')|escape:'html'}" class="{if $jqZoomEnabled && $have_image}jqzoom{/if} img-responsive" title="{if !empty($cover.legend)}{$cover.legend|escape:'htmlall':'UTF-8'}{else}{$product->name|escape:'htmlall':'UTF-8'}{/if}" alt="{if !empty($cover.legend)}{$cover.legend|escape:'htmlall':'UTF-8'}{else}{$product->name|escape:'htmlall':'UTF-8'}{/if}" id="bigpic" width="{$largeSize.width}" height="{$largeSize.height}"/>
		...
		{if isset($images) && count($images) > 3}<span class="view_scroll_spacer"><a id="view_scroll_left" class="hidden" title="{l s='Other views'}" href="javascript:{ldelim}{rdelim}"><i class="fa fa-angle-left"></i></a></span>{/if}
		<div id="thumbs_list">
		...
		{if isset($images) && count($images) > 3}<a id="view_scroll_right" title="{l s='Other views'}" href="javascript:{ldelim}{rdelim}"><i class="fa fa-angle-right"></i></a>{/if}
		</div>
		{/if}
	...
	<!-- left infos-->
	<div id="pb-left-column" class="col-md-7 col-sm-6">
...
<div id="quantityDiscount">
	<table class="std table-responsive">
        <thead>
            <tr>
                <th>{l s='Product'}</th>
                <th>{l s='From (qty)'}</th>
...
<div id="more_info_block" class="block clear box">
	...
	{if isset($accessories) AND $accessories}
		...
		<div id="idTab4" class="bullet">
			<div class="block products_block accessories_block clearfix">
				<div class="block_content product-block">
					{foreach from=$accessories item=accessory name=accessories_list}						
						{if ($accessory.allow_oosp || $accessory.quantity_all_versions > 0 || $accessory.quantity > 0) AND $accessory.available_for_order AND !isset($restricted_country_mode)}
							...
							<div class="col-xs-12 col-md-3 ajax_block_product  product_accessories_description">
								<div class="product-container">
									<div class="image">										
										<a href="{$accessoryLink|escape:'htmlall':'UTF-8'}" title="{$accessory.legend|escape:'htmlall':'UTF-8'}" class="product_img_link">
											<img class="img-responsive" src="{$link->getImageLink($accessory.link_rewrite, $accessory.id_image, 'home_default')|escape:'html'}" alt="{$accessory.legend|escape:'htmlall':'UTF-8'}"  />
										</a>
									</div>
