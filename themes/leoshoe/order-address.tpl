...
{if !$opc}
	...
	{/if}
<form action="{$link->getPageLink($back_order_page, true)|escape:'html'}" method="post" class="form-horizontal">
{else}
...
{/if}
	<div class="addresses clearfix">
		<div  class="address_delivery select form-group clearfix">
			<label class="control-label col-sm-4" for="id_address_delivery">{if $cart->isVirtualCart()}{l s='Choose a billing address:'}{else}{l s='Choose a delivery address:'}{/if}</label>
			<div class="col-sm-4">
		...
		<p class="address_add submit">
			<a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1{if $back}&mod={$back}{/if}")|escape:'html'}" title="{l s='Add'}" class="button">{l s='Add a new address'}</a>
		</p>
		{if !$opc}
		<div id="ordermsg" class="clearfix">
			...
			<label class="txt">{l s='If you would like to add a comment about your order, please write it in the field below.'}</label>
				<textarea class="form-control" rows="4" name="message">{if isset($oldMessage)}{$oldMessage}{/if}</textarea>
			</div>
		</div>
		{/if}
	</div>
