...
<div class="addresses panel panel-default">
	<h3 class="panel-heading">{l s='Your addresses are listed below.'}</h3>
	<div class="panel-body">
		...
		<div class="col-sm-6 col-xs-12">
			<ul class="address {if $smarty.foreach.myLoop.last}last_item{elseif $smarty.foreach.myLoop.first}first_item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{else}item{/if}">
				...
				{/foreach}
				<li class="address_update"><a class=" button" href="{$link->getPageLink('address', true, null, "id_address={$address.object.id|intval}")|escape:'html'}" title="{l s='Update'}">&raquo; {l s='Update'}</a></li>
				<li class="address_delete"><a class=" button" href="{$link->getPageLink('address', true, null, "id_address={$address.object.id|intval}&delete")|escape:'html'}" onclick="return confirm('{l s='Are you sure?' js=1}');" title="{l s='Delete'}">&raquo; {l s='Delete'}</a></li>
			</ul>
		</div>
		{/foreach}
		</div>
...
<div class="clear address_add"><a href="{$link->getPageLink('address', true)|escape:'html'}" title="{l s='Add an address'}" class="button">{l s='Add an address'}</a></div>
</div>
<ul class="footer_links clearfix">
	<li class="f_left"><a class="btn" href="{$link->getPageLink('my-account', true)|escape:'html'}"></span>{l s='Back to your account'}</a></li>
	<li class="f_right"><a class="btn" href="{$base_dir}">{l s='Home'}</a></li>
</ul>
