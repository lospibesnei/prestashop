	...
	<?php
		...
		foreach ($this->_languages as $language)
			echo '<div id="ccontent_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $this->_defaultFormLanguage ? 'block' : 'none').';float: left;">
						<textarea class="rte" cols="80" rows="30" id="content_'.$language['id_lang'].'" name="content_'.$language['id_lang'].'">'.$this->getParamValue('content_'.$language['id_lang'],(isset($defaultC[$language['iso_code']])? $defaultC[$language['iso_code']] : $defaultC['en'])).'</textarea>
					</div>';
