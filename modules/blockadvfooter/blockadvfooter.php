...
/**
 * blockadvfooter Class
 */
require_once( dirname(__FILE__) . '/defines.php' );

class blockadvfooter extends Module {
    ...
    function __construct() {
        global $currentIndex;
        $this->name = 'blockadvfooter';
        parent::__construct();
        $this->tab = 'leotheme';
        $this->author = 'leotheme';
        $this->version = '1.2';
        $this->displayName = $this->l('Advance Footer Module');
        $this->description = $this->l('Advance Footer Module');
    ...
    function install() {
        ...
        $this->_clearCache("blockadvfooter.tpl");
        return true;
    }
    ...
    public function hookHeader($params) {
        ...
        if (version_compare(_PS_VERSION_, "1.5", "<")) {
            Tools::addCSS(($this->_path) . 'blockadvfooter.css', 'all');
        } else {
            $this->context->controller->addCSS(($this->_path) . 'blockadvfooter.css', 'all');
        }
    }
    ...
    private function _getFormConfig() {
        ...
        $link = new Link();
        ob_start();
        include_once dirname(__FILE__) . '/config/blockadvfooter.php';
