<?php /*%%SmartyHeaderCode:19034563ac1548a86c0-59568340%%*/if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_valid = $_smarty_tpl->decodeProperties(array (
  'file_dependency' => 
  array (
    '9ef05545a2b83aa08bb805e69ceffb6765502809' => 
    array (
      0 => 'C:\\wamp\\www\\prestashop\\themes\\leoshoe\\modules\\blockcategories\\blockcategories.tpl',
      1 => 1446822913,
      2 => 'file',
    ),
    '787dc29eb57b697dc38260afff2be2cb3cde7a4a' => 
    array (
      0 => 'C:\\wamp\\www\\prestashop\\themes\\leoshoe\\modules\\blockcategories\\category-tree-branch.tpl',
      1 => 1446822913,
      2 => 'file',
    ),
  ),
  'nocache_hash' => '19034563ac1548a86c0-59568340',
  'cache_lifetime' => 31536000,
  'version' => 'Smarty-3.1.14',
  'unifunc' => 'content_563ff94027eb16_03328989',
  'has_nocache_code' => false,
),true); /*/%%SmartyHeaderCode%%*/?>
<?php if ($_valid && !is_callable('content_563ff94027eb16_03328989')) {function content_563ff94027eb16_03328989($_smarty_tpl) {?>
<!-- Block categories module -->
<div id="categories_block_left" class="block">
	<p class="title_block">Categorías</p>
	<div class="block_content">
		<ul class="tree dhtml">
									
<li >
	<a href="http://localhost/prestashop/index.php?id_category=6&amp;controller=category&amp;id_lang=1" 		title="Complet&aacute; el conjunto de tu equipo y destacate en cada partido, con los Shorts personalizables de Excipio Sport. Una prenda confeccionada en poli&eacute;ster con tecnolog&iacute;a  Riquelme   &nbsp;Fit  , que se encarga de mantenerte m&aacute;s fresco durante la actividad y ofrece mayor comodidad. Adem&aacute;s, posee paneles en  mesh  &nbsp;que aumentan la ventilaci&oacute;n. El modelo presenta cintura reversible con cord&oacute;n de ajuste para personalizar el calce. Un dise&ntilde;o exclusivo con el escudo del club bordado, el logo&nbsp;  Swoosh  &nbsp;y detalles a contratono.">Pantalones</a>
	</li>

												
<li >
	<a href="http://localhost/prestashop/index.php?id_category=4&amp;controller=category&amp;id_lang=1" 		title="Todos los accesorios de moda para tu iPod">Accesorios</a>
	</li>

												
<li >
	<a href="http://localhost/prestashop/index.php?id_category=5&amp;controller=category&amp;id_lang=1" 		title="El &uacute;ltimo procesador Intel, un disco duro m&aacute;s grande, con profusi&oacute;n de memoria y otras novedades. Todo en s&oacute;lo 2,59 cm libres de cualquier obstrucci&oacute;n. Las nuevas port&aacute;tiles Mac cumplir rendimiento, potencia y conectividad de una computadora de escritorio. Sin la parte del escritorio.">Port&aacute;tiles</a>
	</li>

												
<li >
	<a href="http://localhost/prestashop/index.php?id_category=9&amp;controller=category&amp;id_lang=1" 		title="">KAPPA GEN&Eacute;RICA</a>
	</li>

												
<li class="last">
	<a href="http://localhost/prestashop/index.php?id_category=13&amp;controller=category&amp;id_lang=1" 		title="">camisetas</a>
			<ul>
									
<li >
	<a href="http://localhost/prestashop/index.php?id_category=14&amp;controller=category&amp;id_lang=1" class="selected"		title="">Hombre</a>
	</li>

												
<li class="last">
	<a href="http://localhost/prestashop/index.php?id_category=15&amp;controller=category&amp;id_lang=1" 		title="">Mujer</a>
	</li>

							</ul>
	</li>

							</ul>
		
		<script type="text/javascript">
		// <![CDATA[
			// we hide the tree only if JavaScript is activated
			$('div#categories_block_left ul.dhtml').hide();
		// ]]>
		</script>
	</div>
</div>
<!-- /Block categories module -->
<?php }} ?>